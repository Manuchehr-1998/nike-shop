...
const { cart, addToCard, removeFromCart } = inject("cart");
  ...
  try {
    if (!item.isFavorite) {
      ...
      const { data } = await axios.post(
        `https://eb022103d8e1b758.mokky.dev/favorite`,
        obj
      );
      ...
      await axios.delete(
        `https://eb022103d8e1b758.mokky.dev/favorite/${item.favoriteId}`
      );
...
const fetchItems = async () => {
  try {
    ...
    const { data } = await axios.get(
      `https://eb022103d8e1b758.mokky.dev/items`,
      { params }
    );
